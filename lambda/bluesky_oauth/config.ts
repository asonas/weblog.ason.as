...
export async function loadSecret(secretId: string): Promise<BlueskySecret> {
  ...
  if (!response.SecretString) throw new Error("Bluesky OAuth secret has no SecretString");
  const secret = JSON.parse(response.SecretString) as Partial<BlueskySecret>;
  if (!secret.allowed_did || !secret.client_private_jwk || !secret.session_encryption_key) {
