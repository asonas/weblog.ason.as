...
export class EncryptedJson {
  constructor(private readonly key: Buffer) {
    if (key.length !== 32) throw new TypeError("OAuth encryption key must be 32 bytes");
  }
  ...
  decrypt<T>(serialized: string): T {
    ...
    if (envelope.version !== 1) throw new TypeError("Unsupported OAuth ciphertext version");
