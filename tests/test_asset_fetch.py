...
import log_migration.assets as asset_module
    ...
    def do_GET(self):  # noqa: N802
        ...
        if path == "/abc123":
            body = (
                ...
                'content="https://i.gyazo.com/abc123.jpg"></head></html>'
            ).encode()
            self.send_response(200)
...
def test_fetch_assets_resolves_gyazo_page_urls(
    tmp_path: Path,
    local_asset_server: str,
    monkeypatch: pytest.MonkeyPatch,
):
    def local_urlopen(url: str, timeout: float):
        for host in ("https://gyazo.com", "https://i.gyazo.com"):
            if url.startswith(host):
                url = local_asset_server + url.removeprefix(host)
                break
        return real_urlopen(url, timeout=timeout)

    monkeypatch.setattr(asset_module, "urlopen", local_urlopen)
    manifest_path = tmp_path / "asset-manifest.json"
    _write_manifest(
        manifest_path,
        [
    ...
    assert report["downloaded"] == 1
    assert report["failed"] == 0
    assert (tmp_path / "assets" / "asset-gyazo.jpg").read_bytes() == PNG_BYTES
    result = report["results"][0]
    assert result["url"] == "https://gyazo.com/abc123"
    assert result["fetched_url"] == "https://i.gyazo.com/abc123.jpg"
    assert result["sha256"] == hashlib.sha256(PNG_BYTES).hexdigest()
