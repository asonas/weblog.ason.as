...
function exifOrientation(
  ...
  for (let index = 0; index < entries; index += 1) {
    ...
    if (data.getUint16(entry, littleEndian) === 0x0112)
      return data.getUint16(entry + 8, littleEndian);
  }
  return null;
}
...
function jpegDimensions(data: DataView): ImageDimensions | null {
  ...
  while (offset + 8 < data.byteLength) {
    ...
    if (marker === 0xe1)
      orientation = exifOrientation(data, offset, length) ?? orientation;
    if (
      [
...
function webpDimensions(data: DataView): ImageDimensions | null {
  ...
  const ascii = (offset: number, length: number) =>
    String.fromCharCode(
      ...Array.from({ length }, (_, index) => data.getUint8(offset + index)),
    );
  if (ascii(0, 4) !== "RIFF" || ascii(8, 4) !== "WEBP") return null;
  const chunk = ascii(12, 4);
  if (chunk === "VP8X")
    return { width: uint24(data, 24) + 1, height: uint24(data, 27) + 1 };
