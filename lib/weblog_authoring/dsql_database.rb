...
module WeblogAuthoring
  class DsqlDatabase
    ...
    def list_diary_routes
      with_connection do |connection|
        connection.exec(<<~SQL).map { |row| row.fetch("route") }
          SELECT CASE WHEN page_type = 'date' THEN page_date ELSE name END AS route
          FROM #{SCHEMA}.pages
          WHERE status = 'published' AND is_empty = FALSE
            AND EXISTS (SELECT 1 FROM #{SCHEMA}.links WHERE links.source_id = pages.id AND links.target_name = '日記')
        SQL
      end
    end
    ...
    def approved_webmention_counts(page_ids)
      ...
      with_connection do |connection|
        connection.exec_params(<<~SQL, page_ids).to_h { |row| [row.fetch("target_page_id"), row.fetch("count").to_i] }
          SELECT relations.target_page_id, COUNT(DISTINCT relations.id) AS count
          FROM #{SCHEMA}.webmention_relations relations
          JOIN #{SCHEMA}.webmention_snapshots snapshots ON snapshots.relation_id = relations.id
          WHERE relations.target_page_id IN (#{page_ids.each_index.map { |index| "$#{index + 1}" }.join(', ')})
            AND relations.moderation_status = 'approved' AND relations.verification_status = 'verified'
            AND snapshots.snapshot_kind = 'approved' AND snapshots.is_current = TRUE
    ...
    def start_inbox_sync_run(run_id:, trigger:, started_at:, sources:)
      with_connection do |connection|
        connection.transaction do
          ...
          source_parameters = sources.each_index.map { |index| "$#{index + 2}" }.join(", ")
          active = connection.exec_params(
            <<~SQL,
              SELECT runs.id
              FROM #{SCHEMA}.inbox_sync_runs runs
              JOIN #{SCHEMA}.inbox_sync_run_sources run_sources ON run_sources.run_id = runs.id
              WHERE runs.status IN ('queued', 'running')
                AND runs.id <> $1
                AND run_sources.source IN (#{source_parameters})
          ...
          sources.each do |source|
            connection.exec_params(
              <<~SQL,
                ...
                ) VALUES ($1, $2, 'running', 0, 0, 0, 0, NULL, $3)
    ...
    def queue_inbox_sync_run(run_id:, trigger:, queued_at:, sources:)
      with_connection do |connection|
        connection.transaction do
          ...
          source_parameters = sources.each_index.map { |index| "$#{index + 1}" }.join(", ")
          active = connection.exec_params(
            <<~SQL,
              SELECT 1
              FROM #{SCHEMA}.inbox_sync_runs runs
            ...
            connection.exec_params(
              <<~SQL,
                INSERT INTO #{SCHEMA}.inbox_sync_run_sources (
                  run_id, source, status, fetched_count, created_count, updated_count,
                  deleted_count, error, completed_at
                ) VALUES ($1, $2, 'queued', 0, 0, 0, 0, NULL, $3)
              SQL
              [run_id, source, queued_at]
            )
          end
          true
        end
