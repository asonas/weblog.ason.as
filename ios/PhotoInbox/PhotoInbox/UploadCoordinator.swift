...
final class UploadCoordinator {
    ...
    func enqueue(_ photos: [LibraryPhoto]) async {
        do {
            try await store.enqueue(photos.map {
                UploadItem(
    ...
    func processQueue() async {
        ...
        for item in items {
            do {
                ...
                try? await store.updateStage(item.clientUploadID, stage: .failed(message: error.localizedDescription))
                lastError = error.localizedDescription
                scheduleRetry()
            }
            await refreshCount()
    ...
    private func send(_ original: UploadItem, api: MobileAPIClient) async throws {
        ...
        let signed = try await api.createUpload(.init(
